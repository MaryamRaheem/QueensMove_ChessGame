...
class Piece
{
	public:
		...
		void input_position() //inherited in queen 
		{
			
		}
		void set_position() //inherited in queen 
		{
			
		}
		void check_count() //inherited in king 
		{
	...
	public:
		...
		void check_account()
		{
			...
			else if(choice=='n' || choice=='N')
			{
				...
				P1->registeration(); //called from player class 
...
//	getche();
		...
		void Levels()
		{
			...
			if(k.count == choice)
			{
				...
				c.display_board(); //calling chess class ka board function 
				play_game(choice);
			}
			else
			{
		...
		void play_game(int choice) //choice yaani kitni queens k sath khail rahay 
		{
		
			Queen q[choice]; //choice yaani n (jitni queens/nxn board) //array of objects
		...
		int is_valid(int m, int cheQ[][10], int row, int col, int choice)
		{
			...
			//ab check karna hai k jo user na latest position dee hai, usski row and column ma already koi queen tou nai
